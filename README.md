...
一套让 Claude Code 跨会话保持同一套工作方式的配置，打包成可安装的形式。
...
- **「4/4」「0/9」** —— 实测触发率。4 个测试 query 命中 4 次 / 9 次里 0 次命中。
  测试集与原始结果都在 `plugins/hade-skills/evals/`。
...
记忆层补上具体那一次：某个排版 bug 连猜 5 轮全错，直到让用户开 F12 才发现是
Word 残留的 VML 标签卡住了布局。实测：给 skill 加一段这样的具体反例，
同一道色彩计算题的准确率从 0/3 提到 3/3 —— 起作用的不是新知识，是"你上次在这里错过"。
