...
# §十.4 列出的 iOS 锚点。其中绿/靛/紫三色实测 S 分别为 58.6/61.0/68.0%，落在禁区。
# 已裁决（2026-08-24）：锚点是**方向坐标**不是可投产的色值清单（§十.5 原则 5 原文
# 即"iOS 色是坐标不是清单"）。故判定照常以 §十.1 为准、照常报违规 —— 直接照抄
# #34c759 本就该被拦；命中时附注来由，指引"取同色相把 S 提到 70+"。
IOS_ANCHORS = {
...
def main() -> int:
    ...
    for c, hsl in parsed:
        ...
        for e in errs:
            print(f"    → {e}")
        if errs and anchor:
            print(f"    ⓘ 此色是 §十.4 的 iOS 锚点（{anchor}）—— 锚点只指方向，不是可直接投产的色值"
                  f"（§十.5「坐标不是清单」）。取同色相 H≈{H:.0f}°、把 S 提到 70+ 再用")
