...
```bash
node tools/check-context.js          # 人读报告，按章节列出占比
node tools/check-context.js --json   # 机器可读
node tools/check-context.js --repo   # 量仓库副本而非已安装的本体
```

人格层是**无条件加载**的 —— 每个会话、每个项目，在你打第一个字之前就已经进了上下文。
这是 100% 生效的代价，而这个代价此前从没被量过（执行日志里 0 处讨论）。

本工具不替你判断值不值，只把数字和分布摆出来：合计约 2.1–2.8 万 token，
`§八`（30.5%）与 `§九`（27.2%）两章就占骨架的 58%，想瘦身从那里下手。
token 数是**估算**不是实测 —— 没有本地分词器，按中日韩 1 字≈1 token、
拉丁 3.2–4 字≈1 token 推算，给的是区间。
